...
def gen_data_WS02(T, m, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
...
def gen_data_WS08(T, m, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
...
def gen_data_WS0275(T, m, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
...
def run_hyp_new_ERg12(T, m, coeff, q, itera = 1000):
    ...
    R1 = root_network(G_new.subgraph(71), G_new)
    ...
    R5 = root_network(G_new2.subgraph(71), G_new2)
    R6 = root_network(G_new3.subgraph(71), G_new3)
    ...
    # R1=g_3, R4=g_4, R5=g_1, R6=g_2
    
    
    # rej_rate1 = 0
    rej_rate2 = 0

    # g3_prop_exact_match = np.zeros(itera)
    # g3_prop_rd1_match = np.zeros(itera)
    # g3_prop_rd0_match = np.zeros(itera)
    # g4_prop_exact_match = np.zeros(itera)
    # g4_prop_rd1_match = np.zeros(itera)
    # g4_prop_rd0_match = np.zeros(itera)
    g1_prop_exact_match = np.zeros(itera)
    g1_prop_rd1_match = np.zeros(itera)
    g1_prop_rd0_match = np.zeros(itera)
    g2_prop_exact_match = np.zeros(itera)
    g2_prop_rd1_match = np.zeros(itera)
    g2_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data(T, m, coeff, seed = i + 1)

        # # g3 vs g4
        # p1, dist_list14_1, dist_list14_4, nbh_list14_1, nbh_list14_4 = approx_perm(R1.root_nbhd(2), R4.root_nbhd(2), G_list, 2, q, test_level = 0.05)
        # rej_rate1 += p1
        # g3_prop_exact_match[i] = np.mean(dist_list14_1<1/2)
        # g3_prop_rd1_match[i] = np.mean(dist_list14_1<1) - np.mean(dist_list14_1<1/2)
        # g3_prop_rd0_match[i] = np.mean(dist_list14_1>=1)
        # g4_prop_exact_match[i] = np.mean(dist_list14_4<1/2)
        # g4_prop_rd1_match[i] = np.mean(dist_list14_4<1) - np.mean(dist_list14_4<1/2)
        # g4_prop_rd0_match[i] = np.mean(dist_list14_4>=1)

        # g1 vs g2
        p2, dist_list56_5, dist_list56_6, nbh_list56_5, nbh_list56_6 = approx_perm(R5.root_nbhd(2), R6.root_nbhd(2), G_list, 2, q, test_level = 0.05)
        rej_rate2 += p2    
        g1_prop_exact_match[i] = np.mean(dist_list56_5<1/2)
        g1_prop_rd1_match[i] = np.mean(dist_list56_5<1) - np.mean(dist_list56_5<1/2)
        g1_prop_rd0_match[i] = np.mean(dist_list56_5>=1)
        g2_prop_exact_match[i] = np.mean(dist_list56_6<1/2)
        g2_prop_rd1_match[i] = np.mean(dist_list56_6<1) - np.mean(dist_list56_6<1/2)
        g2_prop_rd0_match[i] = np.mean(dist_list56_6>=1)
        

       
    # with open(f"hyp_results{coeff}.csv", 'a') as csvfile:
    #    resultwriter = csv.writer(csvfile, delimiter=',',
    #                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
    #    resultwriter.writerow(['T%d, m%d, q%d'%(T, m, q), rej_rate1/itera, rej_rate2/itera])

    end = timer()

    with open(f"tiebreaking_C{T}_q{q}_ER.txt", 'w') as my_output:
        # my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        my_output.write("g_1 =_d g_2: %f \n" % (rej_rate2/itera))
    ...
    # print("rej rate 1:", rej_rate1/itera)

    rej_rate_str = str(Decimal(float(rej_rate2/itera)*100).quantize(Decimal("0.1"), rounding="ROUND_HALF_UP"))
    print(f"H0:g_1 =_d g_2 (Erdos-Renyi(0.1)), C={T}, q={q}: {rej_rate_str}")
    
    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_1 =_d g_2 (Erdos-Renyi(0.1)), C={T}, q={q}: {rej_rate_str} \n")


def run_hyp_new_RGg12(T, m, coeff, q, itera = 1000):
    ...
    g2_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data_RG(T, m, coeff, seed = i + 1)
    ...
    with open(f"tiebreaking_C{T}_q{q}_RG_g12.txt", 'w') as my_output:
        # my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        my_output.write("g_1 =_d g_2: %f \n" % (rej_rate2/itera))
        # my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g3_prop_exact_match), np.mean(g3_prop_rd1_match), np.mean(g3_prop_rd0_match)))
        # my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
    ...
    # print("rej rate 1:", rej_rate1/itera)
    # print("rej rate 2:", rej_rate2/itera)

    rej_rate_str = str(Decimal(float(rej_rate2/itera)*100).quantize(Decimal("0.1"), rounding="ROUND_HALF_UP"))
    print(f"H0:g_1 =_d g_2 (Random Geometric(0.2)), C={T}, q={q}: {rej_rate_str}")

    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_1 =_d g_2 (Random Geometric(0.2)), C={T}, q={q}: {rej_rate_str} \n")



def run_hyp_new_RGg45(T, m, coeff, q, itera = 1000):
    ...
    R7 = root_network(G_new4.subgraph(4), G_new4)
    ...
    # R7.root_nbhd(2).draw()
    # plt.show()
    # R1=g_3, R4=g_4, R5=g_1, R6=g_2, R7=g_5(g_3 with closure)
    ...
    g4_prop_rd1_match = np.zeros(itera)
    g4_prop_rd0_match = np.zeros(itera)
    g5_prop_exact_match = np.zeros(itera)
    g5_prop_rd1_match = np.zeros(itera)
    g5_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data_RG(T, m, coeff, seed = i + 1)

        # g4 vs g5
        p1, dist_list45_4, dist_list45_5, nbh_list45_4, nbh_list45_5 = approx_perm(R4.root_nbhd(2), R7.root_nbhd(2), G_list, 2, q, test_level = 0.05)
        rej_rate1 += p1
        g4_prop_exact_match[i] = np.mean(dist_list45_4<1/2)
        g4_prop_rd1_match[i] = np.mean(dist_list45_4<1) - np.mean(dist_list45_4<1/2)
        g4_prop_rd0_match[i] = np.mean(dist_list45_4>=1)
        g5_prop_exact_match[i] = np.mean(dist_list45_5<1/2)
        g5_prop_rd1_match[i] = np.mean(dist_list45_5<1) - np.mean(dist_list45_5<1/2)
        g5_prop_rd0_match[i] = np.mean(dist_list45_5>=1)

    end = timer()

    with open(f"tiebreaking_C{T}_q{q}_RG_g45.txt", 'w') as my_output:
        # my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        # my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g3_prop_exact_match), np.mean(g3_prop_rd1_match), np.mean(g3_prop_rd0_match)))
        # my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        # my_output.write("proportion of matches (g1, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g1_prop_exact_match), np.mean(g1_prop_rd1_match), np.mean(g1_prop_rd0_match)))
        # my_output.write("proportion of matches (g2, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g2_prop_exact_match), np.mean(g2_prop_rd1_match), np.mean(g2_prop_rd0_match)))
        # my_output.write("Running time: %f \n" % (end - start))
        my_output.write("g_4 =_d g_5: %f \n" % (rej_rate1/itera))
        my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        my_output.write("proportion of matches (g5, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g5_prop_exact_match), np.mean(g5_prop_rd1_match), np.mean(g5_prop_rd0_match)))
        my_output.write("Running time: %f \n" % (end - start))
        
    # print("rej rate 1:", rej_rate1/itera)

    # print("Running time:", end - start)
    # print("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
    # print("proportion of matches (g5, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g5_prop_exact_match), np.mean(g5_prop_rd1_match), np.mean(g5_prop_rd0_match)))

    rej_rate_str = str(Decimal(float(rej_rate1/itera)*100).quantize(Decimal("0.1"), rounding="ROUND_HALF_UP"))
    print(f"H0:g_4 =_d g_5 (Random Geometric(0.2)), C={T}, q={q}: {rej_rate_str}")
    
    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_4 =_d g_5 (Random Geometric(0.2)), C={T}, q={q}: {rej_rate_str} \n")
        

def run_hyp_new_WS02(T, m, coeff, q, itera = 1000):
    ...
    G_new = gen_data_WS02(20, 20, coeff, seed = 6)[0]
    R4 = root_network(G_new.subgraph(16), G_new)
    R7 = root_network(G_new.subgraph(6), G_new)
    ...
    # R7.root_nbhd(2).draw()
    # plt.show()
    # Here R4 and R7 are g6 and g7 in paper (long forks)
    ...
    g4_prop_rd1_match = np.zeros(itera)
    g4_prop_rd0_match = np.zeros(itera)
    g7_prop_exact_match = np.zeros(itera)
    g7_prop_rd1_match = np.zeros(itera)
    g7_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data_WS02(T, m, coeff, seed = i + 1)

        # g4 vs g7
        p1, dist_list47_4, dist_list47_7, nbh_list47_4, nbh_list47_7 = approx_perm(R4.root_nbhd(2), R7.root_nbhd(2), G_list, 2, q, test_level = 0.05)
        rej_rate1 += p1
        g4_prop_exact_match[i] = np.mean(dist_list47_4<1/2)
        g4_prop_rd1_match[i] = np.mean(dist_list47_4<1) - np.mean(dist_list47_4<1/2)
        g4_prop_rd0_match[i] = np.mean(dist_list47_4>=1)
        g7_prop_exact_match[i] = np.mean(dist_list47_7<1/2)
        g7_prop_rd1_match[i] = np.mean(dist_list47_7<1) - np.mean(dist_list47_7<1/2)
        g7_prop_rd0_match[i] = np.mean(dist_list47_7>=1)
    end = timer()

    with open(f"tiebreaking_C{T}_q{q}_WS_g38.txt", 'w') as my_output:
        # my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        # my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g3_prop_exact_match), np.mean(g3_prop_rd1_match), np.mean(g3_prop_rd0_match)))
        # my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        # my_output.write("proportion of matches (g1, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g1_prop_exact_match), np.mean(g1_prop_rd1_match), np.mean(g1_prop_rd0_match)))
        # my_output.write("proportion of matches (g2, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g2_prop_exact_match), np.mean(g2_prop_rd1_match), np.mean(g2_prop_rd0_match)))
        # my_output.write("Running time: %f \n" % (end - start))
        my_output.write("g_3 =_d g_8: %f \n" % (rej_rate1/itera))
        my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        my_output.write("proportion of matches (g8, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g7_prop_exact_match), np.mean(g7_prop_rd1_match), np.mean(g7_prop_rd0_match)))
        my_output.write("Running time: %f \n" % (end - start))
        
    # print("rej rate 1:", rej_rate1/itera)
    # print("Running time:", end - start)
    # print("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
    # print("proportion of matches (g8, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g7_prop_exact_match), np.mean(g7_prop_rd1_match), np.mean(g7_prop_rd0_match)))

    rej_rate_str = str(Decimal(float(rej_rate1/itera)*100).quantize(Decimal("0.1"), rounding="ROUND_HALF_UP"))
    print(f"H0:g_3 =_d g_8 (Watts-Strogatz(2,0.2)), C={T}, q={q}: {rej_rate_str}")

    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_3 =_d g_8 (Watts-Strogatz(2,0.2)), C={T}, q={q}: {rej_rate_str} \n")
        

def run_hyp_new_WS08(T, m, coeff, q, itera = 1000):
    ...
    G_new = gen_data_WS08(20, 20, coeff, seed = 4)[0]
    R4 = root_network(G_new.subgraph(5), G_new)
    ...
    g7_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data_WS08(T, m, coeff, seed = i + 1)
    ...
    with open(f"tiebreaking_C{T}_q{q}_WS_g67.txt", 'w') as my_output:
        # my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        # my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g3_prop_exact_match), np.mean(g3_prop_rd1_match), np.mean(g3_prop_rd0_match)))
        # my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        # my_output.write("proportion of matches (g1, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g1_prop_exact_match), np.mean(g1_prop_rd1_match), np.mean(g1_prop_rd0_match)))
        # my_output.write("proportion of matches (g2, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g2_prop_exact_match), np.mean(g2_prop_rd1_match), np.mean(g2_prop_rd0_match)))
        # my_output.write("Running time: %f \n" % (end - start))
        my_output.write("g_6 =_d g_7: %f \n" % (rej_rate1/itera))
        my_output.write("proportion of matches (g6, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        my_output.write("proportion of matches (g7, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g7_prop_exact_match), np.mean(g7_prop_rd1_match), np.mean(g7_prop_rd0_match)))
        my_output.write("Running time: %f \n" % (end - start))
        
    # print("rej rate 1:", rej_rate1/itera)
    # print("Running time:", end - start)
    # print("proportion of matches (g6, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
    # print("proportion of matches (g7, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g7_prop_exact_match), np.mean(g7_prop_rd1_match), np.mean(g7_prop_rd0_match)))

    rej_rate_str = str(Decimal(float(rej_rate1/itera)*100).quantize(Decimal("0.1"), rounding="ROUND_HALF_UP"))
    
    print(f"H0:g_6 =_d g_7 (Watts-Strogatz(2,0.8)), C={T}, q={q}: {rej_rate_str}")

    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_6 =_d g_7 (Watts-Strogatz(2,0.8)), C={T}, q={q}: {rej_rate_str} \n")

    

def run_hyp_new_WS0275(T, m, coeff, q, itera = 1000):
    ...
    g2_prop_rd0_match = np.zeros(itera)
    for i in range(itera):
        G_list = gen_data_WS0275(T, m, coeff, seed = i + 1)
    ...
    with open(f"tiebreaking_C{T}_q{q}_WS.txt", 'w') as my_output:
        my_output.write("g_3 =_d g_4: %f \n g_1 =_d g_2: %f \n" % (rej_rate1/itera, rej_rate2/itera))
        my_output.write("proportion of matches (g3, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g3_prop_exact_match), np.mean(g3_prop_rd1_match), np.mean(g3_prop_rd0_match)))
        my_output.write("proportion of matches (g4, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g4_prop_exact_match), np.mean(g4_prop_rd1_match), np.mean(g4_prop_rd0_match)))
        my_output.write("proportion of matches (g1, 1/3,1/2,1): %f, %f, %f \n" % (np.mean(g1_prop_exact_match), np.mean(g1_prop_rd1_match), np.mean(g1_prop_rd0_match)))
    ...
    print(f"H0:g_1 =_d g_2 (Watts-Strogatz(2,0.275)), C={T}, q={q}: {rej_rate_str}")

    with open(f"partial_replication_results.txt", 'a') as my_output:
        my_output.write(f"H0:g_1 =_d g_2 (Watts-Strogatz(2,0.275)), C={T}, q={q}: {rej_rate_str} \n")
...
null = args.input_argument_3

if null == "ERg12":
    run_hyp_new_ERg12(T, m, coeff, q, itera)
elif null == "RGg12":
    run_hyp_new_RGg12(T, m, coeff, q, itera)
elif null == "RGg45":
    run_hyp_new_RGg45(T, m, coeff, q, itera)
elif null == "WS02":
    run_hyp_new_WS02(T, m, coeff, q, itera)
elif null == "WS08":
    run_hyp_new_WS08(T, m, coeff, q, itera)
elif null == "WS0275":
    run_hyp_new_WS0275(T, m, coeff, q, itera)
