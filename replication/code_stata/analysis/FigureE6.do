...
qui reghdfe narrow1_prediction_NS_bin window_pre d_pre3 d_pre2 rand d_0 d_post1 d_post2 d_post3 d_post4 d_post5 window_post DespesasOrçamentárias_PC if city_audit!=., absorb(cod_mun year) cluster(id_state)
coefplot, vertical yline(0, lpattern(dash) lcolor(gray)) drop(window_pre d_post5 window_post _cons DespesasOrçamentárias_PC) xline(3, lpattern(dash) lcolor(gray)) ytitle("Corruption (Budget-Predicted)") xtitle("Years before and after Audit Announcement") xlabel( 1 "-3" 2 "-2" 3 "-1" 4 "0" 5 "1" 6 "2" 7 "3" 8 "4", angle(45)) subtitle("Impact Effect of Audits on Corruption - with budget controls") graphregion(color(white)) color(black) ciopts(recast(rcap) color(black black) lwidth(*0.75 *1.25)) levels(95 90)
graph export "$folder/output/appendix_figures/FigureE6_a.pdf", replace

qui reghdfe narrow1_prediction_NS_bin window_pre d_pre3 d_pre2 rand d_0 d_post1 d_post2 d_post3 d_post4 d_post5 window_post DespesasOrçamentárias_PC if city_narrow_corr==1, absorb(cod_mun year) cluster(id_state)
est store reg1
qui reghdfe narrow1_prediction_NS_bin window_pre d_pre3 d_pre2 d_0 d_post1 d_post2 d_post3 d_post4 d_post5 window_post DespesasOrçamentárias_PC if city_broad_corr==0, absorb(cod_mun year) cluster(id_state)
est store reg2
...
subtitle("Impact Effect of Audits on Corruption: depending on Outcomes  - with budget controls") legend(rows(1)) drop(window_pre d_post5 window_post _cons DespesasOrçamentárias_PC)
graph export "$folder/output/appendix_figures/FigureE6_b.pdf", replace
