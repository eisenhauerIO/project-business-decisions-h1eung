...
def gen_data_ER(T, m, ER_paras, err_bnd, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
        G = nx.erdos_renyi_graph(m, ER_paras)
        for n in G.nodes:
             R = root_network(G.subgraph(n), G)
             ...
             Y = coeff[0]*(deg_0 + 2*clust_0) +\
                 ...
                 np.random.uniform(-err_bnd, err_bnd)
             G.nodes[n]['favour'] = Y
        G_list.append(G)
    return G_list
    
def gen_data_WS(T, m, WS_paras, err_bnd, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
        ...
        G = nx.watts_strogatz_graph(m, 2, WS_paras)
        for n in G.nodes:
             R = root_network(G.subgraph(n), G)
             G1 = R.root_nbhd(1).network
             G2 = R.root_nbhd(2).network
...
def gen_data_RG(T, m, RG_paras, err_bnd, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
        G = nx.random_geometric_graph(m, RG_paras)
        for n in G.nodes:
             R = root_network(G.subgraph(n), G)
             G1 = R.root_nbhd(1).network
             G2 = R.root_nbhd(2).network
...
def run_hyp_alt(T, m, coeff, q, err_bnd, DGP, DGP_paras, itera = 1000):
    ...
    G_new = gen_data_ER(1, 100, 2/100, 5, coeff, seed = 0)[0]
    ...
    for i in range(itera):
        if DGP == "ER":
            G_list = gen_data_ER(T, m, DGP_paras, err_bnd, coeff, seed = i + 1)
        elif DGP == "WS":
            G_list = gen_data_WS(T, m, DGP_paras, err_bnd, coeff, seed = i + 1)
        elif DGP == "RG":
            G_list = gen_data_RG(T, m, DGP_paras, err_bnd, coeff, seed = i + 1)
    ...
    with open(f"tiebreaking_C{T}_q{q}_"+DGP+f"{DGP_paras}_alt_U{err_bnd}.txt", 'w') as my_output:
    ...
    if DGP == "ER":
        print(f"H0:g_3 =_d g_4 (Erdos-Renyi(0.1)), C={T}, q={q}, U in [-{err_bnd},{err_bnd}]: {float(rej_rate1/itera)*100:.1f}")
        with open(f"partial_replication_results.txt", 'a') as my_output:
            my_output.write(f"H0:g_3 =_d g_4 (Erdos-Renyi(0.1)), C={T}, q={q}, U in [-{err_bnd},{err_bnd}]: {float(rej_rate1/itera)*100:.1f} \n")
    elif DGP == "RG":
        print(f"H0:g_3 =_d g_4 (Random Geometric(0.1)), C={T}, q={q}, U in [-{err_bnd},{err_bnd}]: {float(rej_rate1/itera)*100:.1f}")
        with open(f"partial_replication_results.txt", 'a') as my_output:
            my_output.write(f"H0:g_3 =_d g_4 (Random Geometric(0.1)), C={T}, q={q}, U in [-{err_bnd},{err_bnd}]: {float(rej_rate1/itera)*100:.1f} \n")
...
err_bnd = float(args.input_argument_3)
DGP = args.input_argument_4
DGP_paras = float(args.input_argument_5)
itera = int(args.input_argument_6)
m = 20
coeff = [0, 2, 0]


run_hyp_alt(T, m, coeff, q, err_bnd, DGP, DGP_paras, itera)
