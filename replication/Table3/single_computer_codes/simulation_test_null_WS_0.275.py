...
def gen_data_WS(T, m, coeff = [0, 0, 0], seed = 0):
    np.random.seed(seed)
    random.seed(seed)
    G_list = []
    for i in range(T):
...
def run_hyp_new_WS(T, m, coeff, q, itera = 1000):
    ...
    for i in range(itera):
        G_list = gen_data_WS(T, m, coeff, seed = i + 1)
...
run_hyp_new_WS(T, m, coeff, q, itera = 2000)
