...
def return_train_test_split(y_var,
    ...
    # split the indices we have into the folds
...
def do_prediction(y_var, 
                  ...
                  data_option,
                  count):
    prediction_dir = os.getcwd()+f'/data/working/prediction_{model}_{sampling_method}_{data_option}_half/{y_var}_fold{count}'
...
def merged_dataset(model_name,sampling_method,data_option, folds=5):
    ...
    for target in ['corr2','corr1','narrow1']:
        list_of_df = []
...
def produce_cv_predictions(model_name,sampling_method,data_option, folds=5):
    import warnings
    warnings.filterwarnings('ignore')
    warnings.simplefilter('ignore')
    merged_df = merged_dataset(model_name,sampling_method,data_option,folds=2)
    ...
    merged_df.to_csv(wd+f'/data/working/merged/{model_name}_{sampling_method}_{data_option}_half.csv')
    return merged_df
