...
def do_prediction(y_var, 
                  ...
                  count,
                  prediction_dir,
                  hold_out_test_index = None):
    prediction_dir = prediction_dir + f'/{y_var}_fold{count}'
    ...
    if hold_out_test_index is not None:
        budget_data_save[y_var+'_hold_out_test_sample'] = 0
        budget_data_save.loc[hold_out_test_index,y_var+'_hold_out_test_sample'] = 1
        cols_keep = cols_keep + [y_var+'_hold_out_test_sample']
...
def merged_dataset(model_name,sampling_method,data_option, folds=5, targets=['corr2','corr1','narrow1'],prediction_dir=None):
...
def merged_dataset_holdout(model_name,sampling_method,data_option, folds=5, targets=['narrow1'],prediction_dir=None):
    budget_data_raw = pd.read_stata(wd+'/data/final/panel_budget_corruption.dta',
                                    convert_missing=False)
    truth = budget_data_raw[['cod_mun', 'year','corr1','corr2','narrow1']]
    if prediction_dir is None:
        ...
        for count in range(folds):
            ...
            if model_name in ['xgboost','logistic']:
              #this fixed the line 300 
                df[f'{target}_predicted'] = 1 - df[f'{target}_predicted']
            df = df[['cod_mun', 'year', 'for_prediction', 'for_prediction2',
                     f'{target}_predicted',f'{target}_train_sample',f'{target}_hold_out_test_sample']]
            df.columns = ['cod_mun', 'year', 'for_prediction', 'for_prediction2',
                     f'{target}_predicted_{count}',f'{target}_train_sample_{count}',f'{target}_hold_out_test_sample']
            list_of_df.append(df)
        merged_df = list_of_df[0]
        for count in range(1,folds):
            merged_df = merged_df.merge(list_of_df[count].drop(columns = ['for_prediction', 'for_prediction2',f'{target}_hold_out_test_sample']), 
                                        on = ['cod_mun','year'])
        truth = merged_df.merge(truth, on = ['cod_mun', 'year'])
    return truth
...
def produce_cv_predictions_holdout(model_name,sampling_method,data_option, folds=5, t_arr=['corr2','corr1','narrow1'],prediction_dir=None,cv_dir=None):
    import warnings
    warnings.filterwarnings('ignore')
    warnings.simplefilter('ignore')
    merged_df = merged_dataset_holdout(model_name,sampling_method,data_option,folds=folds, targets=t_arr,prediction_dir=prediction_dir)
    print(merged_df.describe())
    for t in t_arr:
        merged_df.loc[:,f'{t}_holdout_prediction'] = np.nan
        if t == 'narrow1':
            for i in range(folds):
                merged_df.loc[merged_df[(merged_df.for_prediction==1)&(merged_df[f'{t}_hold_out_test_sample']==1)].index,f'{t}_hold_out_prediction_{str(i)}'] = merged_df[(merged_df.for_prediction==1)&(merged_df[f'{t}_hold_out_test_sample']==1)][f'{t}_predicted_{str(i)}']
        else:
            for i in range(folds):
                merged_df.loc[merged_df[(merged_df.for_prediction2==1)&(merged_df[f'{t}_hold_out_test_sample']==1)].index,f'{t}_hold_out_prediction_{str(i)}'] = merged_df[(merged_df.for_prediction2==1)&(merged_df[f'{t}_hold_out_test_sample']==1)][f'{t}_predicted_{str(i)}']
    if cv_dir is None:
        cv_dir = wd+f'/data/working/merged/{model_name}_{sampling_method}_{data_option}.csv'
    merged_df.to_csv(cv_dir)
    return merged_df

def produce_cv_predictions(model_name,sampling_method,data_option, folds=5, t_arr=['corr2','corr1','narrow1'],prediction_dir=None,cv_dir=None):
    import warnings
    warnings.filterwarnings('ignore')
    warnings.simplefilter('ignore')
    merged_df = merged_dataset(model_name,sampling_method,data_option,folds=folds, targets=t_arr,prediction_dir=prediction_dir)
    print(merged_df.describe())
    #    cv_df = merged_df[['cod_mun','year','for_prediction','for_prediction2','narrow1','corr1','corr2']]
    for t in t_arr:
        merged_df.loc[:,f'{t}_cv_prediction'] = np.nan
        if t == 'narrow1':
        ...
        merged_df.loc[:,f'{t}_oof_prediction'] = np.nan
        if t == 'narrow1':
            for i in range(folds):
                merged_df.loc[merged_df[(merged_df.for_prediction==1)&(merged_df[f'{t}_train_sample_{str(i)}']!=1)].index,f'{t}_oof_prediction_{str(i)}'] = merged_df[(merged_df.for_prediction==1)&(merged_df[f'{t}_train_sample_{str(i)}']!=1)][f'{t}_predicted_{str(i)}']
        else:
            for i in range(folds):
                merged_df.loc[merged_df[(merged_df.for_prediction2==1)&(merged_df[f'{t}_train_sample_{str(i)}']!=1)].index,f'{t}_oof_prediction_{str(i)}'] = merged_df[(merged_df.for_prediction2==1)&(merged_df[f'{t}_train_sample_{str(i)}']!=1)][f'{t}_predicted_{str(i)}']
    if cv_dir is None:
        cv_dir = wd+f'/data/working/merged/{model_name}_{sampling_method}_{data_option}.csv'
    merged_df.to_csv(cv_dir)
    return merged_df
