...
prediction_dir = os.getcwd()+f'/data/working/prediction_{model_name}_{sampling_method}_{data_option}_fpm'
...
#make dataset
budget_data, X_predict_full = utilities_prediction.preparing_dataset_excluding_fpm(option = data_option,
                                                          test = False)
...
for target in ['narrow1','corr1', 'corr2']:
    for count,fold in enumerate(utilities_prediction.return_train_test_split(y_var = target,
        ...
        best_score,best_p = utilities_prediction.search_hyperparameter(y_var = target,
                                                                ...
                                                                sampling_method = sampling_method)
        utilities_prediction.do_prediction_fpm(y_var = target, 
                                    model = model_name,
                                    train_index = train_index,
