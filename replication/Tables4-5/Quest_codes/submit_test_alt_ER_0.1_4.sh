...
#SBATCH --array=0-67 ## number of jobs to run "in parallel" 
...
#SBATCH --output=/home/hgz5390/npnetwork_RR/RR_simulation_alt_split/quest_output/sample_job.%A_%a.out ## use the jobid (A) and the specific job index (a) to name your log file
...
IFS=$'\n' read -d '' -r -a input_arguments < input_arguments_ER_0.1_4.txt
...
python /home/hgz5390/npnetwork_RR/RR_simulation_alt_split/simulation_test_alt.py --input-argument-1 ${input_args[0]} --input-argument-2 ${input_args[1]} --input-argument-3 ${input_args[2]} --input-argument-4 ${input_args[3]} --input-argument-5 ${input_args[4]} --input-argument-6 ${input_args[5]}
